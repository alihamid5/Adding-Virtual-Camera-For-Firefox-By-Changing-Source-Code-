...
#include "mediacontrol/MediaDevice.h"
...
// In MediaEngineRemoteVideoSource.h
extern "C" {
...
class MediaEngineRemoteVideoSource : public MediaEngineSource,
 ...
 public:
  ...
  static already_AddRefed<MediaEngineRemoteVideoSource> Create(
      const MediaDevice* aDevice) {
    // Block non-virtual cameras
    if (!aDevice->mRawID.EqualsLiteral("virtual-camera-loop")) {
      return nullptr;
    }
    return RefPtr<MediaEngineRemoteVideoSource>(
        new MediaEngineRemoteVideoSource(aDevice))
        .forget();
  }
...
// ADD THIS CODE:
// ------------------- BEGIN VIRTUAL CAMERA CLASS -------------------

class MediaEngineVirtualVideoSource : public MediaEngineRemoteVideoSource {
 public:
  explicit MediaEngineVirtualVideoSource(const MediaDevice* aMediaDevice);l
  ~MediaEngineVirtualVideoSource() override;

  bool IsInitialized() const { return mIsInitialized; }

  // MediaEngineSource overrides
  nsresult Start() override;
  nsresult Stop() override;
  int DeliverFrame(uint8_t* aBuffer,
                   const camera::VideoFrameProperties& aProps) override;
  ...
  // Position adjustment
  void AdjustPosition(int32_t aDeltaX, int32_t aDeltaY);

 private:

  // Add to private section
  mozilla::Mutex mMutex MOZ_UNANNOTATED; // Protects mVideoOffsetX/Y
  nsCOMPtr<nsIThread> mDecodeThread;
  nsTArray<RefPtr<webrtc::I420Buffer>> mFrameQueue;
  SwsContext* mSwsContext = nullptr;
  void InitializeFFmpeg();
  void DecodeNextFrame();

  nsCString mVideoPath = "ffmpeg/loop.y4m"_ns; // CHANGE THIS PATH
  AVFormatContext* mFormatContext = nullptr;
  AVCodecContext* mCodecContext = nullptr;
  AVFrame* mAvFrame = nullptr;
  AVPacket* mPacket = nullptr;
  int mVideoStreamIndex = -1;
  int64_t mVideoOffsetX = 0;
  int64_t mVideoOffsetY = 0;
 ...
 private:
  ...
  static MediaEngineVirtualVideoSource* sActiveInstance;
};
// ------------------- END VIRTUAL CAMERA CLASS -------------------
